...
from .settings import HpsMenuSettings
   
class HpsMenu(TrainingMenu):
    """
    HPS用メニュークラス
    
    通常のトレーニングメニューの情報（重量/レップ数/セット数）に加えて、メニュー種別(筋肥大/パワー/筋力)の情報を持つ
    ...
    def __init__(self,max_weight:Weight,menu_type:MenuTypeEnum,week_number:WeekNumberEnum):
        # メニュー種別
        self.menu_type = menu_type
        # 重量
        weight = self.__choose_weight(max_weight,week_number)
        # レップ数
        reps = self.__choose_reps()
        # セット数
        sets = self.__choose_sets(week_number)
    ...
    def __choose_weight(self,max_weight:Weight,week_number:WeekNumberEnum):
        """
        セットの重量選択メソッド
    ...
    def __choose_ratio(self,week_number:WeekNumberEnum):
        ...
        if(self.__isHypertrophy()):
            return HpsMenuSettings.HYPERTROPHY_WEIGHT_RATIO.get(week_number)  
        # パワーの場合
        if(self.__isPower()):
            return HpsMenuSettings.POWER_WEIGHT_RATIO.get(week_number)  
        # 筋力の場合
        if(self.__isStrength()):
            return HpsMenuSettings.STRENGTH_WEIGHT_RATIO.get(week_number)
    ...
    def __choose_reps(self):
        ...
        # 筋肥大の場合
        if(self.__isHypertrophy()):
            return Reps(HpsMenuSettings.REPS.get(MenuTypeEnum.HYPERTROPHY))   
        # パワーの場合
        if(self.__isPower()):
            return Reps(HpsMenuSettings.REPS.get(MenuTypeEnum.POWER))      
        # 筋力の場合
        if(self.__isStrength()):
            return Reps(HpsMenuSettings.REPS.get(MenuTypeEnum.STRENGTH)) 
        
    def __choose_sets(self,week_number:WeekNumberEnum):
        """
        セット数の選択メソッド
        ...
        # 筋肥大の場合
        if(self.__isHypertrophy()):
            return Sets(HpsMenuSettings.HYPERTROPHY_SETS.get(week_number))     
        # パワーの場合
        if(self.__isPower()):
            return Sets(HpsMenuSettings.POWER_SETS.get(week_number))     
        # 筋力の場合
        if(self.__isStrength()):
            return Sets(HpsMenuSettings.STRENGTH_SETS.get(week_number))     
...
class HpsMenusPerWeek:
    ...
    def __init__(self,max_weight:Weight,week_number:WeekNumberEnum):
       ...
       # 筋肥大の日のメニュー
       self.h_menu = HpsMenu(max_weight,MenuTypeEnum.HYPERTROPHY,week_number)
       # パワーの日のメニュー
       self.p_menu = HpsMenu(max_weight,MenuTypeEnum.POWER,week_number)
       # 筋力の日のメニュー
       self.s_menu = HpsMenu(max_weight,MenuTypeEnum.STRENGTH,week_number)
    def to_dict(self):
        return {
...
class HpsProgram:
    ...
    """
    max_weight:Weight
    hps_menus_for_weeks:Tuple[HpsMenusPerWeek,...]
