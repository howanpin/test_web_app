...
def one_rm_max(request):
    ...
    if request.method == "POST":
        ...
        # 重量計算
        raw_percent_weight = Weight(max_weight.amount * percentage.convert_to_ratio())         # n%の重量を計算
        rounded_percent_weight = raw_percent_weight.round_by_referece_weight(reference_weight) # 基準重量で丸める

        # 戻り値設定
        context = {}
        context['max_weight'] = max_weight                         # 最大重量
        context['percentage'] = percentage                         # パーセンテージ
        context['reference_weight'] = reference_weight             # 基準重量
        context['raw_percent_weight'] = raw_percent_weight         # n%の重量(実測値)
        context['rounded_percent_weight'] = rounded_percent_weight # n%の重量（基準重量で丸め後）
...
def hps_program(request):
    """
    hps_program.htmlのview
    """
    if request.method == "GET":
        # 前回入力値が残っていれば設定してから画面表示
